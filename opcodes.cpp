...
void CPU::Decode(byte opcode) {
  switch(opcode) {
    ...
    case JMPabs:
      JMP(FetchWordPC());
    case JMPind:
      JMP(Fetch(FetchWord(FetchWordPC())));
    ...
    case STAxind:
      STA(Fetch(FetchWord(FetchWordPC()+x)));
    case STAzpg:
      STA(Fetch(FetchPC()));
    case STAabs:
      STA(FetchWordPC());
    case STAindy:
      STA(Fetch(FetchWord(FetchWordPC())+y));
    case STAzpgx:
      STA(Fetch(FetchPC()+x));
    case STAabsy:
      STA(Fetch(FetchWordPC()+y));
    case STAabsx:
      STA(Fetch(FetchWordPC()+x));
