#version 440 core

layout(location = 0) out vec4 colour;

in vec2 txco;

uniform sampler2D tex;

const int colours[] = {
    , 
};


void main() {
  vec2 t = txco;
  t.y = 1 - t.y;
  colour.r = float(colours[int(3*255*texture(tex, t).r)]) / 255;
  colour.g = float(colours[int(3*255*texture(tex, t).r) + 1]) / 255;
  colour.b = float(colours[int(3*255*texture(tex, t).r) + 2]) / 255;
  colour.a = 1;
}
